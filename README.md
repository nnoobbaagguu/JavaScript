...
<p>비교연산자에는 일단 ===, &lt, &gt가 있다.이 연산자들은 모두 이항 연산자이다.<br>
===는 비교 연산자라고 부르며, 왼쪽의 피연산자와 오른쪽의 피연산자가 같은지 판단한다. 두 값이 같으면 true를 반환하고, 두 값이 다르면 false를 반환한다.<br>
&lt, &gt는 두 값의 크기를 비교하는 연산자이다. HTML에서는 이 기호가 태그를 표현할 때 사용되므로 다른 표기를 사용한다. JavaScript 코드를 작성할 때는 &lt, &gt를 그대로 사용해도 된다. </p>
